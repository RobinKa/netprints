...
{
    public class FilteredObservableCollection<TFiltered, TOriginal> : IList<TFiltered>, ICollection, IReadOnlyList<TFiltered>, INotifyCollectionChanged where TFiltered : TOriginal
    {
        ...
        public bool IsSynchronized => ((ICollection)subset).IsSynchronized;

        public object SyncRoot => ((ICollection)subset).SyncRoot;

        TFiltered IList<TFiltered>.this[int index]
        {
            get => subset[index];
        ...
        public void Add(TFiltered item)
        {
            Debug.Assert(filter(item), "Tried to add item that does not match the filter.");
            original.Add(item);
        }
        ...
        public void Clear()
        {
            ...
            Debug.Assert(subset.Count == 0, "Subset not empty after clear.");
        }

        public bool Contains(TFiltered item) => subset.Contains(item);

        public void CopyTo(TFiltered[] array, int arrayIndex)
        {
            throw new NotImplementedException();
        }

        public bool Remove(TFiltered item)
        {
            if (subset.Contains(item))
            {
                bool removed = original.Remove(item);

                Debug.Assert(removed, "Subset contains item but could not be removed from original.");
        ...
        public void Insert(int index, TFiltered item)
        {
            throw new NotImplementedException();
        }
